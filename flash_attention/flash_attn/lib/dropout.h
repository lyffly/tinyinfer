...
struct Dropout {
    ...
    __forceinline__ __device__ Dropout(const unsigned long long seed,
                                       ...
                                       const uint8_t p_dropout_in_uint8_t, const int bid,
                                       const int hid, const int tid, const int nheads)
        : seed(seed),
    ...
    __forceinline__ __device__ void apply_dropout(Tensor<Engine, Layout>& tensor_,
        ...
        Tensor tensor =
            make_tensor(tensor_.data(), flash::convert_layout_acc_dropout(tensor_.layout()));
...
#pragma unroll
            for (int n = 0; n < size<2>(tensor) / 2; ++n, ++rowcol.y) {
                ...
                uint4 random_uint4 =
                    flash::philox(seed, reinterpret_cast<unsigned long long&>(rowcol), offset);
                ...
                if (!encode_dropout_in_sign_bit && (std::is_same<T, cutlass::half_t>::value ||
...
#pragma unroll
                        for (int i = 0; i < 8; i++) {
                            tensor(i, m, n * 2 + j) = encode_dropout(
                                rnd_8[j * 8 + i] <= p_dropout_in_uint8_t, tensor(i, m, n * 2 + j));
                        }
                        Tensor tensor_uint32 = recast<uint32_t>(tensor(_, m, n * 2 + j));
                        // if (cute::thread0()) { printf("tensor_uint32 = 0x%x, 0x%x, 0x%x, 0x%x\n", tensor_uint32(0), tensor_uint32(1), tensor_uint32(2), tensor_uint32(3)); }
                    }
