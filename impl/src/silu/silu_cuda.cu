...
__global__ void silu_cuda_kernel_fp32_float2(const size_t count, const float2* input,
                                             float2* output) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= count)
        return;
...
bool silu_cuda_backend(int64_t in_ptr, int64_t out_ptr, std::vector<int> in_shape,
    ...
    if (dtype == "float32") {
        ...
        if (length % 2 == 0) {
            ...
            silu_cuda_kernel_fp16<<<grid_size, block_size, 0, stream>>>(length, (const half*)in_ptr,
                                                                        (half*)out_ptr);
        }
    } else {
