...
template<> __device__ inline float activation_op<Activation_Relu, float>(float a,  float alpha, float beta) {
    return (a > 0) ? a : 0;
}
template<> __device__ inline float activation_op<Activation_Sigmoid, float>(float a,  float alpha, float beta) {
    return 1.f / (1.f + expf(-a));
}
template<> __device__ inline float activation_op<Activation_LeakyRelu, float>(float a,  float alpha, float beta) {
