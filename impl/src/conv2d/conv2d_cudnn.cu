...
bool conv2d_backend(int64_t in_ptr, int64_t weight_ptr, int64_t bias_ptr, int64_t out_ptr, int64_t workspace_size, 
    ...
    if (dtype == "float32") {
        add_conv2d_bias_fp<float><<<grid_size, block_size, 0, stream>>>((float*)out_ptr, (float*)bias_ptr,  batch, outc, hw, length);
    } else if (dtype == "float16") {
        add_conv2d_bias_fp<half><<<grid_size, block_size, 0, stream>>>((half*)out_ptr, (half*)bias_ptr,  batch, outc, hw, length);
    }
