...
__global__ void ppl_cukernel_pooling_max_common(const T* input, T* output, int batch,
                                                int pad_channels, int in_height, int in_width,
                                                int out_height, int out_width, int kernel_height,
...
bool pooling_cuda_backend(int64_t in_ptr, int64_t out_ptr, std::vector<int> kernels,
                          std::vector<int> paddings, std::vector<int> strides,
                          std::vector<int> in_shape, std::vector<int> out_shape, std::string optype,
                          std::string dtype, std::string layout, int64_t pstream, void* desc) {
    ...
    if (optype == "MaxPool") {
        ...
        dim_grid.z = batch;
        if (dtype == "float32") {
            ppl_cukernel_pooling_max_common<4, 1, float>
                <<<dim_grid, dim_block, 0, (cudaStream_t)pstream>>>(
                    (float*)in_ptr, (float*)out_ptr, batch, inc, inh, inw, outh, outw,
                    kernels.at(0), kernels.at(1), strides.at(0), strides.at(1), paddings.at(0),
                    paddings.at(1));
        } else {
            ppl_cukernel_pooling_max_common<4, 1, __half>
                <<<dim_grid, dim_block, 0, (cudaStream_t)pstream>>>(
                    (__half*)in_ptr, (__half*)out_ptr, batch, inc, inh, inw, outh, outw,
                    kernels.at(0), kernels.at(1), strides.at(0), strides.at(1), paddings.at(0),
                    paddings.at(1));
        }
        return true;
