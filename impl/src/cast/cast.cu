...
template <>
__device__ inline float cast_op<half, float>(half input) {
    return __float2half(input);
}
template <>
__device__ inline half cast_op<float, half>(float input) {
